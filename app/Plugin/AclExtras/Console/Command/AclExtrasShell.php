 ...
 */
	function main() {
		$out  = __("Available ACO sync commands:") . "\n";
		$out .= "\t - aco_update\n";
		$out .= "\t - aco_sync\n";
		$out .= "\t - recover \$type\n";
		$out .= "\t - verify \$type\n";
 ...
 */
	function _checkNode($path, $alias, $parentId = null) {
		...
		if (!$node) {
			...
			$this->out(sprintf(__('Created Aco node: %s'), $path));
		} else {
			$node = $node[0];
		}
		return $node;
			...
			foreach ($actionNodes as $action) {
				if (!isset($methodFlip[$action['Aco']['alias']])) {
					...
					if ($this->Aco->delete()) {
						...
						$this->out(sprintf(__('Deleted Aco node %s'), $path));
					}
				}
			}
		}
 ...
 */
	function help() {
		$head  = __("Usage: cake acl_extras <command>") . "\n";
		$head .= "-----------------------------------------------\n";
		$head .= __("Commands:") . "\n\n";

		$commands = array(
			'update' => "\tcake acl_extras aco_update\n" .
						"\t\t" . __("Add new ACOs for new controllers and actions") . "\n" .
						"\t\t" . __("Create new ACO's for controllers and their actions. Does not remove any nodes from ACO table"),

			'sync' =>	"\tcake acl_extras aco_sync\n" .
						"\t\tPerform a full sync on the ACO table.\n" .
						"\t\t" . __("Creates new ACO's for missing controllers and actions. Removes orphaned entries in the ACO table.") . "\n",

			'verify' => "\tcake acl_extras verify \$type\n" .
						"\t\t" . __('Verify the tree structure of either your Aco or Aro Trees'),

			'recover' => "\tcake acl_extras recover \$type\n" .
						 "\t\t" . __('Recover a corrupted Tree'),
 ...
 */
	function recover() {
		...
		if ($return === true) {
			...
			$this->out(__('Tree recovery failed.'));
		}
	}
}
?>