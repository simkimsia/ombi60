 ...
 **/
	function logDate($created) {
		if ($this->Time->isToday($created)) {
 			$date = "<strong>" . __('Today', true) . "</strong>";
		} elseif ($this->Time->wasYesterday($created)) {
			$date = __('Yesterday', true);
		} else {
 			$date = $this->Time->format('M d', $created);
		}
 ...
 */
	...
	function logAt($log) {
  	$user_name = (isset($log['User']['name'])) ? $log['User']['name'] : __('System', true);
	  return 'at ' . $this->Time->format('h:i a', $log['Log']['created']) . ' ' . __('by', true) . ' ' . $user_name;
	}
	
	function logAction($log) {
		switch ($log['action']) {
			case 'add' :
				$message = __('created', true);
				break;
			case 'delete' :
