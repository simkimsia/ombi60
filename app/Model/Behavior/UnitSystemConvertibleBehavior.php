 ...
 */
	public function convertForDisplay(&$Model, $dataArray=array(), $unit = null, $primary=true) {
		...
		if ($weight_exists) {
			...
			$mainPrimaryDataArray['displayed_weight'] = $this->precision($result_weight, 1);
				
		}

		if ($height_exists) {
			$result_height = $mainPrimaryDataArray['height'] * $lengthMultiplier;
			$mainPrimaryDataArray['displayed_height'] = $this->precision($result_height, 1);
				
		}

		if ($length_exists) {
			$result_length = $mainPrimaryDataArray['length'] * $lengthMultiplier;
			$mainPrimaryDataArray['displayed_length'] = $this->precision($result_length, 1);
		}
	...
	private function convertOtherWeightFieldsForDisplay($alias, $dataArray, $weightMultiplier) {
		...
		foreach($this->settings[$alias]['weight_fields'] as $weightField){
			...
			if ($field_exists) {
				...
				$dataArray['displayed_' . $weightField] = $this->precision($result_weight, 1);
			}
		}
	...
	private function convertOtherWeightFieldsForSave($alias, $dataArray, $weightMultiplier) {
		...
		foreach($this->settings[$alias]['weight_fields'] as $weightField){
			...
			if ($field_exists) {
				...
				$dataArray[$alias][$weightField] = $this->precision($result_weight, 0);
			}
		}
	...
	public function convertForSave(&$Model, $dataArray=array(), $unit = null) {
		...
		if ($weight_exists) {
			...
			$dataArray[$alias]['weight'] = $this->precision($result_weight, 0);
			
                } 
		
		if ($height_exists) {
			$result_height = $dataArray[$alias]['displayed_height'] / $lengthMultiplier;
                        $dataArray[$alias]['height'] = $this->precision($result_height, 0);
			
                }
		
		if ($length_exists) {
			$result_length = $dataArray[$alias]['displayed_length'] / $lengthMultiplier;
                        $dataArray[$alias]['length'] = $this->precision($result_length, 0);
                }
