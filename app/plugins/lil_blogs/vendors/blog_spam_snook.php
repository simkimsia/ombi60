...
class BlogSpamSnook extends Object {
    ...
    function categorize($c) {
      ...
      $score += $comments->find('count', array('conditions' => array('Comment.email'=>$c['email'], 'Comment.status'=>2)));
      $score -= $comments->find('count', array('conditions' => array('Comment.email'=>$c['email'], 'Comment.status'=>0)));
      $score -= $comments->find('count', array('conditions' => array('Comment.ip'=>$c['ip'], 'Comment.status'=>0)));
