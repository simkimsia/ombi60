...
<div class="post" id="post-<?php echo $post['Post']['id']; ?>">
	...
	<div class="meta">
		<?php
			...
			if (!empty($post['Author'][Configure::read('LilBlogsPlugin.authorDisplayField')])) {
				printf(__d('lil_blogs', 'Posted %1$s by %2$s', true),
					$time->timeAgoInWords($post['Post']['created']),
					$post['Author'][Configure::read('LilBlogsPlugin.authorDisplayField')]
				);
			} else {
	...
	<div class="body"><?php
		$body = $post['Post']['body'];
		if ($excerpt_length = Configure::read('LilBlogsPlugin.excerptLength')) {
			$body = $text->truncate($body, $excerpt_length, '...', false, true);
		} else if (stripos($body, Configure::read('LilBlogsPlugin.excerptDelimiter'))!==false) {
			$body = substr($body, 0, stripos($body, Configure::read('LilBlogsPlugin.excerptDelimiter')));
		}
