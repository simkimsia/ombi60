	...
	<div>
		<?php
			if ($auth->user(Configure::read('LilBlogsPlugin.authorAdminField'))) {
				echo $html->link($html->image('/lil_blogs/img/add.gif').' '.__d('lil_blogs', 'Create a new Blog', true), array('action'=>'admin_add'), null, null, false);
			}
		?>
	</div>
</div>
<div class="index">
	...
	?>
		<tr<?php if($i++%2==0) echo ' class="altrow"'; ?>>
			...
			<td class="center"><?php echo $html->link($html->image('/lil_blogs/img/posts.gif', array('alt'=>__d('lil_blogs', 'Show Posts', true))), array('controller'=>'posts', 'action'=>'admin_index', $item['Blog']['id']), array('title'=>__d('lil_blogs', 'Show Posts', true)), null, false); ?></td>
		</tr>
	<?php } ?>
	</table>
</div>