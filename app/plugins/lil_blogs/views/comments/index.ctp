...
?>
	<div class="comment" id="c<?php echo $comment['Comment']['id']; ?>">
		<div class="meta">
			<?php 
				echo $html->link(__d('lil_blogs', 'Comment on', true).' "'.$comment['Post']['title'].'" '.(empty($blog['Blog']['name'])?' '.__d('lil_blogs', 'on', true).' "'.$comment['Post']['Blog']['name'].'" ':'').__d('lil_blogs', 'by', true).' '.$comment['Comment']['author'],
				array('admin'=>false, 'plugin'=>'lil_blogs', 'controller'=>'posts', 'action'=>'view', 'blogname'=>$blog['Blog']['short_name'], 'post'=>$comment['Post']['slug'].'#c'.$comment['Comment']['id']));
			?>
